...
async function makeLogger(charName = "Character", instance = "UI") {
  return new Promise((resolve, reject) => {
    const now = new Date();
    const dateStr = now.getFullYear() + "-" + (now.getUTCMonth() + 1).toString().padStart(2, "0") + "-" + now.getDate().toString().padStart(2, "0");
    ...
    fs.open(fileFullPath, "a", (err, fd) => {
      ...
      function log(str) {
        if (!logging) return console.error("Error, tried to log after unloading logger!");
